...
def test_if_checkout_information_is_shown(is_logged_in, finalize_order, checkout_page):
...
def test_if_order_is_done(is_logged_in, finalize_order, checkout_page):
    checkout_page.finish()

    expect(checkout_page.page.get_by_text(ORDER_CONFIRMATION_MESSAGE)).to_be_visible()

def test_checkout_with_missing_information(is_logged_in, finalize_with_invalid_information, checkout_page):
